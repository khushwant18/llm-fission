...
_CONFIG_FOR_DOC = "GptOssConfig"

# Add docstring constants
GPTOSS_START_DOCSTRING = r"""
...
@add_start_docstrings(
    """
    The GptOss Model transformer with a language modeling head on top (linear layer with weights tied to the input
    embeddings).
    """,
    GPTOSS_START_DOCSTRING,
)
class GptOssForCausalLM(MixtralForCausalLM):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> model = GptOssForCausalLM.from_pretrained("your-username/gpt-oss-model")
        >>> tokenizer = AutoTokenizer.from_pretrained("your-username/gpt-oss-model")
    ...
    def prepare_inputs_for_generation(
        ...
        if cache_position is None:
            cache_position = torch.arange(0, input_length, device=input_ids.device)
        elif cache_position[-1] >= input_length:
            cache_position = torch.arange(cache_position[-1] - input_length + 1, cache_position[-1] + 1, device=input_ids.device)
...
@add_start_docstrings(
    ...
    [`GptOssForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
class GptOssForSequenceClassification(GptOssPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
